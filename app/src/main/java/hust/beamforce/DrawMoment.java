...
public class DrawMoment extends View {
    ...
    public DrawMoment(Context context, int scope, int offset) {
        super(context);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //绘制坐标
        canvas.drawLine(5*step, offsetY, 45*step, offsetY, paint);
        canvas.drawLine(5*step,2*step,5*step,30*step,paint);
        path.moveTo(4.5f*step,30*step);
        path.lineTo(5.5f*step,30*step);
        path.lineTo(5*step,32*step);
        path.close();
        canvas.drawPath(path,paint);
        path.reset();
        canvas.drawText("M",1*step,30*step,paint);

        //绘制曲线
        float points[][] = MainActivity.beam.pointForDraw();

        float max=Math.abs(points[1][0]);
        for(float point : points[1]){
            if(Math.abs(point) > max){
                max = Math.abs(point);
            }
        }

        path.moveTo(5*step,offsetY + (points[1][0]/max)*(offsetY - 4*step));
        for( int i=0;i<=1000;i++){
            float x = 5*step + i*(40*step)/1000;
            float y = offsetY + (points[1][i]/max)*(offsetY - 4*step);
            path.lineTo(x,y);
        }
